...
exports['default'] = _lodash2['default'].extend({}, _BaseGraphTester2['default'], {
  testWeightedDegree: function testWeightedDegree() {
    ...
    assert.deepEqual(_Array$from(G.degree(null, 'other').values()), [3, 7, 4]);
  ...
  testNodeAttr2: function testNodeAttr2() {
    ...
    assert.deepEqual(G.nodes(true), [[0, {}], [1, {}], [2, {}], [3, { 'foo': 'bar' }]]);
  },
  ...
  testEdgeAttr2: function testEdgeAttr2() {
    ...
    assert.deepEqual(G.edges(true).sort(), [[1, 2, { 'foo': 'foo' }], [3, 4, { 'foo': 'foo' }]]);
  },

  testEdgeAttr3: function testEdgeAttr3() {
    var G = new this.Graph();
    G.addEdgesFrom([[1, 2, { 'weight': 32 }], [3, 4, { 'weight': 64 }]], { foo: 'foo' });
    assert.deepEqual(G.edges(true), [[1, 2, { 'foo': 'foo', 'weight': 32 }], [3, 4, { 'foo': 'foo', 'weight': 64 }]]);
  ...
  testEdgeAttr4: function testEdgeAttr4() {
    ...
    assert.deepEqual(G.edges(true), [[1, 2, { 'data': 20, 'spam': 'bar',
  ...
  testSelfloopsAttr: function testSelfloopsAttr() {
    ...
    assert.deepEqual(G.selfloopEdges(true), [[0, 0, {}], [1, 1, { weight: 2 }]]);
  }
});
