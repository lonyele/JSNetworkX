...
var testMap = {
  ...
  '#set() && #get()': {
    ...
    'integers and strings are treated separately': function integersAndStringsAreTreatedSeparately() {
      var map = new _Map2['default']();
      map.set(0, 1);
      map.set('0', 2);
    ...
    'arrays of primitive are considered equal': function arraysOfPrimitiveAreConsideredEqual() {
      var map = new _Map2['default']();
      map.set([1, 2], 1);
    ...
    'object keys with same toString result are considered equal': function objectKeysWithSameToStringResultAreConsideredEqual() {
      var map = new _Map2['default']();
      var obj1 = { toString: function toString() {
          return 'foo';
        } };
      var obj2 = { toString: function toString() {
          return 'bar';
        } };
      var obj3 = { toString: function toString() {
          return 'bar';
        } };
  ...
  '#entries()': function entries() {
    assert(_regeneratorRuntime.isGeneratorFunction(this.map.entries), 'is generator');
    assert.deepEqual(_Array$from(this.map.entries()).sort(), [['0', 0], ['1', 1], ['2', 2]]);
  },

  '#keys()': function keys() {
    assert(_regeneratorRuntime.isGeneratorFunction(this.map.keys), 'is generator');
    assert.deepEqual(_Array$from(this.map.keys()).sort(), ['0', '1', '2']);
  },

  '#values()': function values() {
    assert(_regeneratorRuntime.isGeneratorFunction(this.map.values), 'is generator');
