...
var TestMultiDiGraph = _Object$assign({}, _MultiGraphTest.TestMultiGraph, _BaseMultiDiGraphTester2['default'], {
  ...
  testAddEdge: function testAddEdge() {
    ...
    G.addEdge(0, 1);
    assert.deepEqual(G.adj, new Map({ 0: new Map({ 1: { 0: {} } }), 1: new Map() }));
    assert.deepEqual(G.succ, new Map({ 0: new Map({ 1: { 0: {} } }), 1: new Map() }));
    assert.deepEqual(G.pred, new Map({ 0: new Map(), 1: new Map({ 0: { 0: {} } }) }));
