...
var TestGraph = _lodash2['default'].extend({}, _BaseAttrGraphTester2['default'], {
  ...
  testAddNodesFrom: function testAddNodesFrom() {
    ...
    assert.deepEqual(G.adj, new Map([[0, new Map()], [1, new Map()], [2, new Map()]]));
    // test add attributes
  ...
  testAddEdgesFrom: function testAddEdgesFrom() {
    ...
    assert.deepEqual(G.adj, new Map([[0, new Map([[1, {}], [2, { weight: 3 }]])], [1, new Map([[0, {}]])], [2, new Map([[0, { weight: 3 }]])]]));
    G.addEdgesFrom([[0, 1], [0, 2, { weight: 3 }], [1, 2, { data: 4 }]], { data: 2 });
    assert.deepEqual(G.adj, new Map([[0, new Map([[1, { data: 2 }], [2, { data: 2, weight: 3 }]])], [1, new Map([[0, { data: 2 }], [2, { data: 4 }]])], [2, new Map([[0, { weight: 3, data: 2 }], [1, { data: 4 }]])]]));
    assert.throws(function () {
      G.addEdgesFrom([[0]]);
    }, _exceptionsJSNetworkXError2['default']);
    assert.throws(function () {
